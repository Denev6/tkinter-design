...
class App(Tk):
    ...
    def switch_frame(self, frame_class, *parameters):
        new_frame = frame_class(self, *parameters)
...
class InfoPage(Toplevel): 
   def __init__(self, master):
        ...
        Label(self, text='TEXT', bg='#ffffff', font=(None, 12)).pack(fill=BOTH, expand=True)
...
class IntroPage(MainFrame):
    def __init__(self, master):
        ...
        button = Button(self, image=photo_button, relief='flat', bg=self._bg, command=lambda: master.switch_frame(StartPage))
...
class StartPage(MainFrame):
    def __init__(self, master):
        ...
        icon.pack(side='left')
        Label(f_title, text='Basic Info', bg=self._bg, font=(None, 14, 'bold'), anchor='w').pack(pady=30, fill='x')
        ...
        Label(f_left, bg=self._bg, text='OPTION', font=self._font, fg='#404040').pack(pady=17, anchor='e', padx=13)
        Label(f_left, bg=self._bg, text='ENTRY_1', font=self._font, bd=0, fg='#404040').pack(pady=10, anchor='e', padx=13)
        Label(f_left, bg=self._bg, text='ENTRY_2', font=self._font, bd=0, fg='#404040').pack(pady=10, anchor='e', padx=13)
        Label(f_left, bg=self._bg, text='ENTRY_3', font=self._font, bd=0, fg='#404040').pack(pady=10, anchor='e', padx=13)
        combo = ttk.Combobox(f_right, textvariable=combo_var, font=self._font, width=14, state="readonly")
        combo['values'] = ['OPTION_A' , 'OPTION_B', 'OPTION_C']
        combo.current(0)
        combo.pack(pady=16, anchor='w')
        entry_1 = Entry(f_right, textvariable=entry_1_var, font=self._font, width=15, relief='flat', bg=self._green2)
        entry_1.pack(pady=10, anchor='w')
        entry_2 = Entry(f_right, textvariable=entry_3_var, font=self._font, width=15, relief='flat', bg=self._green2)
        entry_2.pack(pady=10, anchor='w')
        entry_3 = Entry(f_right, textvariable=entry_2_var, font=self._font, width=15, relief='flat', bg=self._green2)
        ...
        Label(f_btn, text='아래 버튼을 이용해 데이터를 열기/저장할 수 있습니다.', font=(None, 8), bg=self._bg).pack(fill='x')
        image = get_img('button-submit', 'design')
        photo_submit = ImageTk.PhotoImage(image)
        btn_right = Button(f_btn, bg=self._bg, image=photo_submit, relief='flat', \
                            activebackground=self._bg, command=lambda: self.switch_to_ResultPage(master))
        ...
        btn_save = Button(f_btn, bg=self._bg, image=photo_save, relief='flat', width=width_save, height=height_save, \
                            activebackground=self._bg, command=lambda: save_new_csv(entry_1, entry_2, entry_3))
        ...
        btn_open = Button(f_btn, bg=self._bg, image=photo_file, relief='flat', width=width_file, height=height_file, \
                            activebackground=self._bg, command=lambda: open_csv(entry_1, entry_2, entry_3))
        btn_open.image = photo_file
        btn_open.pack(side='right', pady=20)
        self._menu = Menu(master)
        self._menu_data = Menu(self._menu, tearoff=0)
        self._menu_data.add_command(label='OPEN', command=lambda: open_csv(entry_1, entry_2, entry_3))
        self._menu_data.add_command(label='SAVE', command=lambda: save_new_csv(entry_1, entry_2, entry_3))
...
class ResultPage(MainFrame):
    def __init__(self, master, texts, imgs, urls):
        ...
        Button(f_1, text=texts[0], relief='flat', bg='#ffffff', font=self._font, activebackground='#ffffff', \
                command=lambda: show_ImgPage(ImgPage, master, imgs[0])).pack(fill='x', pady=7)
        Button(f_1, text=texts[1], relief='flat',  bg='#ffffff', font=self._font, activebackground='#ffffff', \
                command=lambda: show_ImgPage(ImgPage, master, imgs[1])).pack(fill='x', pady=7)
        Button(f_1, text=texts[2], relief='flat',  bg='#ffffff', font=self._font, activebackground='#ffffff', \
                command=lambda: show_ImgPage(ImgPage, master, imgs[2])).pack(fill='x', pady=7)
        Button(f_1, text=texts[3], relief='flat',  bg='#ffffff', font=self._font, activebackground='#ffffff', \
                command=lambda: show_ImgPage(ImgPage, master, imgs[3])).pack(fill='x', pady=7)
        Button(f_1, text=texts[4], relief='flat',  bg='#ffffff', font=self._font, activebackground='#ffffff', \
                command=lambda: show_ImgPage(ImgPage, master, imgs[4])).pack(fill='x', pady=7)
        ...
        Button(f_2, text=texts[0], relief='flat',  bg='#ffffff', font=self._font, activebackground='#ffffff', \
                command=lambda: open_url(urls[0])).pack(fill='x', pady=13)
        Button(f_2, text=texts[1], relief='flat',  bg='#ffffff', font=self._font, activebackground='#ffffff', \
                command=lambda: open_url(urls[1])).pack(fill='x', pady=13)
        Button(f_2, text=texts[2], relief='flat',  bg='#ffffff', font=self._font, activebackground='#ffffff', \
                command=lambda: open_url(urls[2])).pack(fill='x', pady=13)
        ...
        f_text = Text(f_3, bg='#ffffff', wrap='word', spacing1=10, spacing2=2, relief='flat', \
                            font=(None, 10), yscrollcommand=scrollbar.set)
        ...
        width, height = image.size
        btn_right = Button(f_btn, image=photo_right, bg=self._green, relief='flat', width=width, height=height, \
                            activebackground=self._green, command=lambda: master.switch_frame(OptionalPage, texts, imgs, urls))
...
class OptionalPage(MainFrame):
    def __init__(self, master, texts, imgs, urls):
        ...
        Label(f_title, text='Optional Info', bg=self._bg, font=(None, 14, 'bold'), anchor='w').pack(pady=10, fill='x')
        f_left = Frame(self, bg=self._bg) 
        f_right = Frame(self, bg=self._bg)
        f_btn = Frame(self, bg=self._bg)
        ...
        Label(f_left, bg=self._bg, text='COMBO_1', font=self._font, fg='#404040').pack(padx=18, pady=20, anchor='e')
        ...
        Label(f_color, bg=self._bg, text='COMBO_2', font=self._font, fg='#404040').pack(pady=20, side='right', fill='x')
        tip_box = InfoBalloon(master)
        tip_box.bind_widget(tip_mark, balloonmsg='Here we have Info')
        Label(f_left, bg=self._bg, text='COMBO_3', font=self._font, fg='#404040').pack(padx=18, pady=20, anchor='e')
        combobox_1 = ttk.Combobox(f_right, textvariable=combo_1, font=self._font, state="readonly")
        combobox_2 = ttk.Combobox(f_right, textvariable=combo_2, font=self._font, state="readonly")
        combobox_3 = ttk.Combobox(f_right, textvariable=combo_3, font=self._font, state="readonly")
        ...
        point_line.image = photo_line
        point_line.pack(pady=10)
        Label(f_btn, text='? 위에 마우스를 올려 정보를 볼 수 있습니다.', font=(None, 8), bg=self._bg).pack(fill='x')
        ...
        btn_right = Button(f_btn, image=photo_right, bg='#ffffff', relief='flat', width=width_right, height=height_right, \
                  activebackground=self._bg, command=lambda: self.switch_to_OptionaResultPage(master, urls))
        btn_left = Button(f_btn, image=photo_left, bg='#ffffff', relief='flat', width=width_left, height=height_left, \
                  activebackground=self._bg, command=lambda: master.switch_frame(ResultPage, texts, imgs, urls))
        btn_right.image = photo_right
        btn_right.pack(side='right', padx=25, pady=20)
...
class OptionResultPage(MainFrame):
    def __init__(self, master, urls, texts, imgs, colors, texts_):
        ...
        Button(f_season, text=texts[0], bg='#ffffff', relief='flat', font=self._font, activebackground='#ffffff', \
                        command=lambda: show_ImgPage(ImgPage, master, imgs[0])).pack(fill='x', pady=7)
        Button(f_season, text=texts[1], bg='#ffffff', relief='flat', font=self._font, activebackground='#ffffff', \
                        command=lambda: show_ImgPage(ImgPage, master, imgs[1])).pack(fill='x', pady=7)
        Button(f_season, text=texts[2], bg='#ffffff', relief='flat', font=self._font, activebackground='#ffffff', \
                        command=lambda: show_ImgPage(ImgPage, master, imgs[2])).pack(fill='x', pady=7)
        Button(f_season, text=texts[3], bg='#ffffff', relief='flat', font=self._font, activebackground='#ffffff', \
                        command=lambda: show_ImgPage(ImgPage, master, imgs[3])).pack(fill='x', pady=7)
        ...
        f_image.image = photo_img
        f_image.pack(side='left')
        Label(f_ref_season, text='* click to \nshow images.', bg='#ffffff').pack(pady=7, side='bottom')
        notebook.add(f_color, text='sample_2')
        Label(f_color, bg='#ffffff', text='Colors', relief='flat', font=self._font).pack(anchor='w', padx=10, pady=18, expand=True)
        Label(f_color, bg=colors[0], height=4, relief='groove').pack(fill='x', padx=10, pady=10, expand=True)
        Label(f_color, bg=colors[1], height=4, relief='groove').pack(fill='x', padx=10, pady=10, expand=True)
        Label(f_color, bg=colors[2], height=4, relief='groove').pack(fill='x', padx=10, pady=10, expand=True)
        notebook.add(f_situation, text='sample_3')
        scrollbar = Scrollbar(f_situation, orient='vertical')
        scrollbar.pack(side="right", fill="y")
        f_text = Text(f_situation, bg='#ffffff', wrap='word', spacing1=10, spacing2=3, spacing3=10, font=(None, 11), yscrollcommand=scrollbar.set)
        f_text.pack(side="left", fill="both")
        scrollbar.config(command=f_text.yview)
        f_text.delete(1.0, END)
        ...
        photo_left = ImageTk.PhotoImage(image_left)
        width_left, height_left = image_left.size
        btn_quit = Button(f_btn, image=photo_quit, bg=self._green, relief='flat', width=width_quit, height=height_quit, \
                  activebackground=self._green, command=master.quit)
        tip_box = InfoBalloon(master)
        tip_box.bind_widget(btn_quit, balloonmsg='quit')
        btn_left = Button(f_btn, image=photo_left, bg=self._green, relief='flat', width=width_left, height=height_left, \
                  activebackground=self._green, command=lambda: master.switch_frame(OptionalPage, texts, imgs, urls))
